...
int ws_getclientsock(ws_cli_conn_t *client) {
	if (!CLIENT_VALID(client))
		return (NULL);
...
int ws_close_client(ws_cli_conn_t *client) {
	...
	clse_code[0] = (cc >> 8);
	clse_code[1] = (cc & 0xFF);
	if (ws_sendframe(client, (const char *)clse_code, sizeof(char) * 2, WS_FR_OP_CLSE) <
		0) {
 ...
 * for completeness.
 */
static int read_frame(struct ws_frame_data *wfd, int opcode, unsigned char **buf,
	uint64_t *frame_length, uint64_t *frame_size, uint64_t *msg_idx, uint8_t *masks,
	int is_fin) {
	...
	*frame_size += *frame_length;
	...
	if (*frame_length > 0) {
		if (!is_control_frame(opcode)) {
			tmp =
				realloc(msg, sizeof(unsigned char) * (*msg_idx + *frame_length + is_fin));
			if (!tmp) {
	...
	do {
		...
		if (opcode == WS_FR_OP_TXT || opcode == WS_FR_OP_BIN || opcode == WS_FR_OP_CONT ||
			...
			if (opcode == WS_FR_OP_TXT || opcode == WS_FR_OP_BIN ||
				opcode == WS_FR_OP_CONT) {
				if (read_frame(wfd, opcode, &msg_data, &frame_length, &wfd->frame_size,
						&msg_idx_data, masks_data, is_fin) < 0)
					break;
				...
				if (wfd->frame_type == WS_FR_OP_TXT) {
					if (is_fin) {
						if (is_utf8_len_state(msg_data + (msg_idx_data - frame_length),
					...
					/* Check current state for a CONT or initial TXT frame. */
						...
						utf8_state =
							is_utf8_len_state(msg_data + (msg_idx_data - frame_length),
								frame_length, utf8_state);
...
int ws_socket(
	...
	*sock = socket(AF_INET, SOCK_STREAM, 0);
	...
	reuse = 1;
	if (setsockopt(*sock, SOL_SOCKET, SO_REUSEADDR, (const char *)&reuse, sizeof(reuse)) <
		0) {
