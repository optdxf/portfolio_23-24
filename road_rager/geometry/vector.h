...
/**
 * Returns the unit vector of a vector (same direction but w/ magnitude 1)
 * @param v the vector whose unit vector to return
 * @return a new vector w/ the same direction as @p v but w/ magnitude 1
 */
vector_t vec_unit_vec(vector_t v);

/**
 * Determines the angle between two vectors, from [0,2*pi).
 * @param v1 the first vector
 * @param v2 the second vector
 * @return the angle between v1 and v2 in radians, from [0,2*pi) measured from v1
 */
